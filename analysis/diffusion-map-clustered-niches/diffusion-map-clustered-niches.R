...
niche.points <- matrix( runif(2*n.points, 0.1, .9), nrow = 2, ncol = n.points) 
sig <- matrix(c(0.001, 0, 0, 0.001), 2, 2)
cent <- apply(niche.points, MARGIN = 2, mvrnorm, n = 10, Sigma = sig, simplify = F)
cent <- do.call(rbind, cent)
niche$centroid <- t(cent)
...
# Plot
png(filename = paste(here(), "/figures/diffusion-map-clustered-niches/",
...
write.csv(dj.summary, file = paste(here(), 
                                   "/analysis/diffusion-map-clustered-niches/",
