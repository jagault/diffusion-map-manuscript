...
# Adjust niche width as a function of env
# niche$l1 <- niche$l1 * 1/(1 - niche$centroid[1, ])
# niche$l2 <- niche$l2 * 1/(1 - niche$centroid[1, ])
# 
# niche$l1 <- niche$l1 * (1 + niche$centroid[1, ]*2)
# niche$l2 <- niche$l2 * (1 + niche$centroid[1, ]*2)
for (i in 1:n.spec){
  if (niche$centroid[1, i] >= 0.5){
    niche$l1[i] <- niche$l1[i]*2
    niche$l2[i] <- niche$l2[i]*2
  } else {
    niche$l1[i] <- niche$l1[i]/2
    niche$l2[i] <- niche$l2[i]/2
  }
