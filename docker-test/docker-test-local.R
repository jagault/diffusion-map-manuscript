...
test.lap.local <- site.lap[[1]]$laplacian
saveRDS(test.lap.local, file = paste(here(), "/docker-test",
                                     "/test-lap-local.rds", sep = ""))

# Read in laplacian from local machine
test.lap.remote <- readRDS(paste(here(), "/docker-test",
...
site.dim1.local <- site.dim[sim == 1, ]
saveRDS(site.dim1.local, file = paste(here(), "/docker-test",
                                      "/site-dim1-local.rds", sep = ""))

# Read in remote site dim
site.dim1.remote <- readRDS(paste(here(), "/docker-test",
...
site.dim1.remote[, dim1 := dim1*-1]
site.dim1.remote[, dim2 := dim2*-1]
